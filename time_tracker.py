...
parameters = {'include_time_stats': True, 'per_page': 100, 'page': 0}

# urls and templates
issues_url_template = "{base_url}/projects/{project_id}/issues"
...
while True:
    parameters["page"] += 1
    # call API for 100 issues on page
    issues = requests.get(issue_notes_url, headers=headers, params=parameters)
    # loop through all issues in chronological order
    for issue in issues.json()[::-1]:

        issue_notes_url = issue_notes_url_template.format(base_url=base_url, project_id=project_id,
                                                          issue_iid=issue['iid'])
        # call API for all notes under issue
        issue_notes_response = requests.get(issue_notes_url, headers=headers, params={'include_time_stats': True})
    ...
    if 'next' in issues.links:
        issue_notes_url = issues.links['next']['url']
    else:
        break
